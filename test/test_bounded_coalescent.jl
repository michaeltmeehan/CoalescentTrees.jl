...
@testset "Forward and Backwards Probability Calculator" begin
    ...
    lineages, backwards_probs = sample_lineages_backward(forward_probs, sampled_sequences, sequence_times, bound_time, Ne)
...
@testset "Partition Intervals" begin
    ...
    fp = calc_forward_probs(sampled_sequences, sequence_times, bound_time, Ne)
    lineages, _ = sample_lineages_backward(fp, sampled_sequences, sequence_times, bound_time, Ne)

    times, new_lineages, new_samples = partition_intervals(lineages, sampled_sequences, sequence_times, bound_time, Ne)
...
@testset "Sample Coalescent Time" begin
    ...
    for n_small in [2, 5, 10]
        times = [sample_coalescent_time(t_lower, t_upper, n_small, Ne) for _ in 1:10_000]
        @test all(t_lower .≤ times .≤ t_upper)
    end

    # histogram([sample_coalescent_time(0.0, 1.0, 5, 10.0) for _ in 1:10_000], bins=50)

    # Degenerate interval
    t_fixed = sample_coalescent_time(2.5, 2.5, 5, Ne)
    @test t_fixed == 2.5
end
...
@testset "Sample Topology" begin
    ...
    @test all(tree[node].left .!= tree[node].right for node in eachnode(tree) if !is_leaf(tree, node))
end